...
use crate::types_structs::{Genotype, GenotypeCount, GnPosition, Haplotype, SnpPosition};
...
pub fn get_partition_stats_ref_wild(
    ...
    for i in 0..ploidy {
        ...
        for frag in partition[i].iter() {
            let ((same_ref, diff_ref), (same_wild, diff_wild)) =
                utils_frags::distance_read_haplo_ref_wild(frag, haplo);
            errors_ref += diff_ref;
...
pub fn estimate_epsilon(
    ...
    for i in random_vec.into_iter() {
        ...
        let block = utils_frags::hap_block_from_partition(&part, true);
    ...
    log::debug!(
        "Average count partitions from local clustering : {:?}",
        median_part_stats
    );
...
pub fn estimate_ploidy_flopp(
    ...
    for i in 0..num_ploidies - 1 {
        ...
        log::debug!(
            ...
            mec_threshold,
            mec_vector[i + 1] as f64 / mec_vector[i] as f64
        );
